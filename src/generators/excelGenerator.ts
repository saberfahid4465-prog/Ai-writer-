...
export async function generateExcel(
  ...
  if (!excelData || typeof excelData !== 'object') {
    throw new Error('Excel generation failed: Invalid excelData');
  }
  if (!metaData || typeof metaData !== 'object') {
    throw new Error('Excel generation failed: Invalid metaData');
  }

  // Safe property access with fallbacks
  const safeTitle = typeof metaData.title === 'string' ? metaData.title : 'AI Writer Output';
  ...
  const safeHeaders = Array.isArray(excelData.headers) ? excelData.headers : ['Section', 'Key Points', 'Image Keyword'];
  const safeRows = Array.isArray(excelData.rows) ? excelData.rows : [];

  const workbook = new ExcelJS.Workbook();

  // Detect RTL language for cell alignment
  const RTL_LANGUAGES = ['arabic', 'hebrew', 'urdu', 'persian', 'farsi'];
  const isRTL = RTL_LANGUAGES.some(l => safeLanguage.toLowerCase().includes(l));
  const dataAlignment = isRTL ? 'right' as const : 'left' as const;

  // Set workbook metadata
  workbook.creator = safeAuthor;
  workbook.created = new Date();
  workbook.modified = new Date();
  workbook.lastModifiedBy = 'AI Writer';

  // ─── Main Data Sheet ─────────────────────────────────────
  const sheet = workbook.addWorksheet(sanitizeSheetName(safeTitle), {
    properties: {
      defaultColWidth: 25,
    },
  });
  ...
  const titleRow = sheet.addRow([safeTitle]);
  if (safeHeaders.length > 1) {
    sheet.mergeCells(1, 1, 1, safeHeaders.length);
  }
  titleRow.font = {
    name: FONT_NAME,
    size: 16,
  ...
  const subtitleRow = sheet.addRow([
    `Generated by ${safeAuthor} | Language: ${safeLanguage}`,
  ]);
  if (safeHeaders.length > 1) {
    sheet.mergeCells(2, 1, 2, safeHeaders.length);
  }
  subtitleRow.font = {
    name: FONT_NAME,
    size: 10,
  ...
  // ─── Header Row ──────────────────────────────────────────
  const headerRow = sheet.addRow(safeHeaders);
  headerRow.height = 28;
  ...
  sheet.columns.forEach((column, i) => {
    let maxLength = safeHeaders[i]?.length || 10;
    safeRows.forEach((row) => {
      const safeRow = Array.isArray(row) ? row : [];
      const cellLength = safeRow[i]?.toString().length || 0;
